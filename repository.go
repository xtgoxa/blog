...
func (r *PostRepository) Delete(id int64) error {
	...
	_, _ = r.db.Exec("DELETE FROM comments WHERE post_id = ?", id)
...
// CountByCategory returns number of posts by category
func (r *PostRepository) CountByCategory(category string) (int, error) {
	var count int
	err := r.db.QueryRow("SELECT COUNT(*) FROM posts WHERE category = ?", category).Scan(&count)
	if err != nil {
		return 0, err
	}
	return count, nil
